...
### 5.4 Gemma 4 Multimodal (Direct ASR — No Whisper)

| Property | Value |
|----------|-------|
| **Backend** | `transformers.AutoModelForMultimodalLM` |
| **Model Path** | `models/models--google--gemma-4-E4B-it/snapshots/...` |
| **Approach** | Audio numpy array → downsample to 16kHz → LLM generates text directly |
| **Strengths** | No separate ASR model needed, single-step transcription, very good precision |
| **Weaknesses** | Requires significant GPU memory |

This is the most elegant approach in the project: audio is loaded as raw numpy arrays, resampled to 16kHz, and passed directly to Gemma 4. The model handles audio understanding and Persian transcription in one pass — no Whisper, no intermediate text stage.
...
| ریسک بالا پاداش پاین از باز خود متنفرم | ریسک بالا، پاداش پایین. از این وضعیت متنفرم. |
...
**Key insight:** This evaluation uses **Gemma 4 multimodal directly** — no separate ASR model. The audio is read as a numpy array, downsampled to 16kHz, and fed directly to Gemma 4 for transcription. The LLM handles both audio understanding and text generation in a single step, achieving very good precision without any Whisper dependency.
...
3. Computes WER/CER against ground-truth transcriptions
4. Output: enriched parquet + Excel report
...
2. **Gemma 4 multimodal** can transcribe directly without any ASR model — audio is loaded as numpy arrays, downsampled to 16kHz, and converted to text by the LLM alone. This approach achieves very good precision and eliminates the Whisper dependency entirely
...
- **[Ali Sadeghi-Aghili](https://github.com/alisadeghiaghili)**

- **[Melika Bahman-Abadi](https://github.com/melikabahmanabadi)**

- MohammadReza Gahtarani
...
- [Iranian License Plate Generator](https://github.com/barzansaeedpour/iranian-license-plate-generator)
